...
class EnemyMissileMT : public EnemyBase
{
public:
	// アニメーション種別
	...
	// コンストラクタ
	EnemyMissileMT(const EnemyBase::EnemyData& data);
	// デストラクタ
	~EnemyMissileMT(void) override;
...
private:
	...
	static constexpr float SHOT_INTERVAL = 4.0f; // n秒に1回発射

	// バースト射撃用パラメータ
	int burstCount_ = 0;                   // 現在何発まで撃ったか
	float burstDelayTimer_ = 0.0f;         // 弾と弾の間のディレイ用タイマー

	static constexpr int BURST_SHOT_NUM = 4;      // 1回につき何連射するか（3点バースト）
	static constexpr float BURST_DELAY = 0.15f;    // 連射時の弾間隔
