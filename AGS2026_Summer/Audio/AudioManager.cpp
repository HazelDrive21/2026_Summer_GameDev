...
void AudioManager::LoadSceneSound(LoadScene scene)
{
	...
	case LoadScene::MENU:  table = &SoundTable_Menu::Table; break;
...
void AudioManager::PlaySELoop(SoundID id)
{
	auto it = handles_.find(id);
	if (it == handles_.end()) return;

	// すでに再生中なら、重ねて再生しないようにガード
	if (CheckSoundMem(it->second) == 1) return;

	int volume = static_cast<int>(seVolume_ * (masterVolume_ / 255.0f));
	ChangeVolumeSoundMem(volume, it->second);

	// 🔥 第2引数を DX_PLAYTYPE_LOOP にしてループ再生！(頭出しはFALSE)
